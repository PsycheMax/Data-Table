...
import { FooterCell, HeaderCell, RowData, TableData } from '../types/DataTypes.js';
import { TableConf } from '../types/ConfTypes.js';
import { generateID, sortRowsByIdsArray } from '../utils/index.js';
import { RenderedCellObject, RenderedRowObject, RenderedSubRowObject } from '../types/RenderTypes.js';
...
export class DataTable extends LitElement {
   ...
   */
  @state()
  private additionalColumnsProperties: Array<AdditionalColumnProperties> = ['data-table-alerts', 'data-table-chevron', 'data-table-settings',
  ];
  ...
  */
  ...
  reorderRenderedRows(propertyToReorderBy: string, inverted = false): void {
    ...
    const intermediateArray = this.renderedRows.map((row, index) => {
      const targetCell = row.cellObjects.filter((element: RenderedCellObject) => element.property === propertyToReorderBy)[0];
    ...
    intermediateArray.forEach((element) => this.conf.renderOrder.rows.customOrder?.push(element.rowID));
    this.requestUpdate();
  }
  ...
  reorderRenderedColumns(newPropertyOrder?: Array<string>) {
    if (this.conf.renderOrder?.columns) {
      if (newPropertyOrder) {
        ...
        this.conf.renderOrder.columns.customOrder = this.conf.renderOrder.columns.defaultOrder;
  ...
  toggleColumn(columnID: string) {
    if (this.conf.renderOrder.columns.hiddenIDs === undefined) {
      ...
      this.conf.renderOrder.columns.hiddenIDs.splice(this.conf.renderOrder.columns.hiddenIDs.indexOf(columnID), 1);
    } else {
  ...
  generateHeader(): TemplateResult {
    if (this.conf.headerRow !== undefined) {
      ...
      if (!this.isRowDefaultOrderExternal) this.conf.renderOrder.rows.defaultOrder = [];
      if (!this.isRowCustomOrderExternal) this.conf.renderOrder.rows.customOrder = [];
      ...
      Object.keys(this.conf.headerRow?.cells).forEach(key => {
        if (this.conf?.headerRow?.cells[key]) {
          ...
          if (this.conf.renderOrder.columns?.hiddenIDs?.includes(headerRowCell.id)) {
      ...
      this.additionalColumnsProperties.forEach((additionalProperty) => {
        if (this.conf.renderOrder.columns.defaultOrder && !this.conf.renderOrder.columns.defaultOrder.includes(additionalProperty)) this.conf.renderOrder.columns.defaultOrder.push(additionalProperty);
        if (this.conf.renderOrder.columns.customOrder !== undefined && !this.conf.renderOrder.columns.customOrder.includes(additionalProperty)) this.conf.renderOrder.columns.customOrder.push(additionalProperty);
      });
      if (!this.isRowDefaultOrderExternal) this.conf.renderOrder.rows.defaultOrder.push(rowID);
      if (!this.isRowCustomOrderExternal) this.conf.renderOrder.rows.customOrder?.push(rowID);
      return html`
        <tr class="row header" id=${rowID}>
          ${this.renderRowDataContent(this.conf.headerRow, this.conf.renderOrder.columns.customOrder, true)}
        </tr>
      `;
    }
    return html``;
    ...
    if (this.conf.footerRow !== undefined) {
      ...
      Object.keys(this.conf.footerRow.cells).forEach(key => {
        if (this.conf?.footerRow?.cells[key]) {
          ...
          if (this.conf.renderOrder.columns?.hiddenIDs?.includes(footerRowCell.id)) {
      ...
      this.additionalColumnsProperties.forEach((additionalProperties) => {
        if (this.conf.renderOrder.columns.defaultOrder && !this.conf.renderOrder.columns.defaultOrder.includes(additionalProperties)) this.conf.renderOrder.columns.defaultOrder.push(additionalProperties);
        if (this.conf.renderOrder.columns.customOrder !== undefined && !this.conf.renderOrder.columns.customOrder.includes(additionalProperties)) this.conf.renderOrder.columns.customOrder.push(additionalProperties);
      });
      if (!this.isRowDefaultOrderExternal) this.conf.renderOrder.rows.defaultOrder.push(rowID);
      if (!this.isRowCustomOrderExternal) this.conf.renderOrder.rows.customOrder?.push(rowID);
      return html`
        <tr class="row footer" id=${rowID}>
          ${this.renderRowDataContent(this.conf.footerRow, this.conf.renderOrder.columns.customOrder, false, false)}
        </tr>
      `;
    }
    return html``;
  ...
  private renderDataContent(): TemplateResult {
    ...
    if (this.data) {
      ...
      const rowsDefaultOrder = this.isRowDefaultOrderExternal ? this.conf.renderOrder.rows.defaultOrder : [];
      const rowsCustomOrder = this.isRowCustomOrderExternal && this.conf.renderOrder.rows.customOrder ? this.conf.renderOrder.rows.customOrder : [];
      const columnsDefaultOrder = this.isColumnDefaultOrderExternal ? this.conf.renderOrder.columns.defaultOrder : [];
      const columnsCustomOrder = this.isColumnCustomOrderExternal && this.conf.renderOrder.columns.customOrder ? this.conf.renderOrder.columns.customOrder : [];

      const isVariantWide = this.conf?.styling?.table?.areRowsSeparated ?? false;
      const paddingRow = isVariantWide ? html`<tr class="padding-row"></tr>` : null;
      ...
      this.data?.forEach((rowDataObject) => {
        ...
        if (rowDataObject.cells) {
          Object.keys(rowDataObject.cells).forEach(key => {
            if (rowDataObject.cells?.[key]) {
              ...
              const valueToString = itemInRow.value !== undefined ? itemInRow.value.toString() : null;
              if (!this.isColumnDefaultOrderExternal && !columnsCustomOrder.includes(itemInRow.property)) columnsDefaultOrder.push(itemInRow.property);
              if (!this.isColumnCustomOrderExternal && !columnsCustomOrder.includes(itemInRow.property)) columnsCustomOrder.push(itemInRow.property);
              rowCellObjects.push({
        ...
        // Subrows rendering
        if (rowDataObject.subRows !== undefined && rowDataObject.subRows.length > 0) {
          ...
          rowDataObject.subRows?.forEach((subRow, index) => {
            const isLastOne = rowDataObject.subRows && index === rowDataObject.subRows.length - 1;
            let subrowGeneratedClass = `row sub-row ${variantClassName}`;
            subrowGeneratedClass += isLastOne ? 'last-subrow' : 'intermediate-subrow';
            subrowGeneratedClass += !rowDataObject.areSubRowsOpen || subRow.isHidden ? ' hidden' : null;
            ...
            const subRowToAdd: RenderedSubRowObject = {
              ...
              subRowResult: html`
                <tr
                  ...
                  id="${rowID}"
                >
                  ${this.renderRowDataContent(subRow, this.conf.renderOrder.columns.customOrder, false, subRow.hasCustomSubrowContent)}
                </tr>
              `
            };
        ...
        if (rowToRender.hasPreview && rowDataObject.previewRow !== undefined) {
          ...
          previewConditionalClass += rowDataObject.areSubRowsOpen ? 'intermediate-subrow ' : 'last-subrow ';
          rowToRender.preview = {
        ...
        conditionalClass += !rowToRender.preview?.isHidden ? 'preview-open ' : 'preview-hidden ';
        conditionalClass += rowToRender.hasSubrows ? 'with-subrows ' : '';
        conditionalClass += rowToRender.hasSubrows && rowDataObject.areSubRowsOpen ? 'subrows-open ' : 'subrows-hidden ';
        ...
        rowToRender.result = html`
          <tr
            ...
            id="${rowID}"
          >
            ${this.renderRowDataContent(rowDataObject, this.conf.renderOrder.columns.customOrder, false)}
          </tr>
          ${!rowToRender.preview?.isHidden ? rowToRender.preview?.content : null}
      ...
      if (rowsDefaultOrder.length > 0) this.conf.renderOrder.rows.defaultOrder = rowsDefaultOrder;
      if (rowsCustomOrder.length > 0) this.conf.renderOrder.rows.defaultOrder = rowsCustomOrder;
      if (columnsDefaultOrder.length > 0) this.conf.renderOrder.columns.defaultOrder = columnsDefaultOrder;
      if (columnsCustomOrder.length > 0) this.conf.renderOrder.columns.customOrder = columnsCustomOrder;
      ...
      const orderedRows = sortRowsByIdsArray(toRenderRows, this.conf.renderOrder.rows.customOrder ?? this.conf.renderOrder.rows.defaultOrder);
  ...
  renderRowDataContent(
    ...
    if (isSubrowAndHasCustomContent) {
      ...
      return html`
      <td colspan="9999">
        <slot name=${rowData.id}></slot>
      </td>`;
    }

    if (!this.conf.renderOrder.rows.defaultOrder.includes(rowData.id)) this.conf.renderOrder.rows.defaultOrder.push(rowData.id);
    if (!this.conf.renderOrder.rows.customOrder?.includes(rowData.id)) this.conf.renderOrder.rows.customOrder?.push(rowData.id);

    let tableSettingsCell: IDataCellEntry;
    let threeDotsCell: IDataCellEntry;
    let alertCell: IDataCellEntry;
    if (this.conf.hasCustomThreeDotsCell || this.conf.hasCustomSettingsCell || this.conf.hasCustomAlertCell) {
      if (rowData.cells) {
        Object.keys(rowData.cells).forEach((key) => {
          const cell = rowData.cells?.[key];
          if (cell?.isSettingsCell) {
            threeDotsCell = { ...cell, property: this.additionalColumnsProperties[2] };
            if (isHeader) tableSettingsCell = { ...cell, property: this.additionalColumnsProperties[2] };
            if (!this.additionalColumnsProperties.includes(key as AdditionalColumnProperties)) {
              delete rowData.cells?.[key];
            }
          }
          if (cell?.isAlertCell) {
            alertCell = { ...cell, property: this.additionalColumnsProperties[0] };
            if (!this.additionalColumnsProperties.includes(key as AdditionalColumnProperties)) {
              delete rowData.cells?.[key];
            }
          }
    ...
    if (this.conf.hasAlerts !== undefined) shouldRenderAdditionalProperties = true;
    if (this.conf.hasSettings !== undefined) shouldRenderAdditionalProperties = true;
    if (this.conf.hasSubrows !== undefined) shouldRenderAdditionalProperties = true;
        ...
        switch (additionalProperty) {
          ...
          case this.additionalColumnsProperties[0]:
            if (this.conf.hasAlerts) {
              ...
              if (this.conf.hasCustomAlertCell) {
                ...
                const isAlertButtonHidden = isHeader ? true : rowData.hasAlertsToShow === undefined;
                const onAlert = rowData.rowOnAlertClickFunction !== undefined ? rowData.rowOnAlertClickFunction : this.onTableAlertClickFunction;
                rowData.cells = {
                  ...rowData.cells,
                  [`${additionalProperty}`]: {
                    ...additionalPropertyCommons,
          ...
          case this.additionalColumnsProperties[1]:
            if (this.conf.hasSubrows) {
              ...
              const onChevron = () => {
                if (!this.preTableChevronClickFunction && !this.onTableChevronClickFunction && !rowData.rowPreChevronFunction && !rowData.rowOnChevronFunction) {
                  rowData.areSubRowsOpen = !rowData.areSubRowsOpen;
                  this.requestUpdate();
                };
                if (this.preTableChevronClickFunction !== undefined) {
                  this.preTableChevronClickFunction();
                } else {
                  rowData.rowPreChevronFunction?.();
                }
                if (this.onTableChevronClickFunction !== undefined) {
                  this.onTableChevronClickFunction();
                } else {
                  rowData.rowOnChevronFunction?.();
                }
              };
              const isChevronHiddenValue = rowData.isChevronHidden ?? (rowData.subRows === undefined || rowData.subRows.length === 0);
              rowData.cells = {
                ...rowData.cells,
                [`${additionalProperty}`]: {
                  ...additionalPropertyCommons,
                  id: `${rowData.id}-chevron`,
                  isChevronCell: true,
          ...
          case this.additionalColumnsProperties[2]:
            if (this.conf.hasSettings) {
              ...
              if (!this.conf.hasCustomThreeDotsCell && !isHeader) {
                const onThreeDots = rowData.rowOnThreeDotsFunction ?? this.onTableThreeDotsClickFunction;
                rowData.cells = {
                  ...rowData.cells,
                  [`${additionalProperty}`]: {
                    ...additionalPropertyCommons,
                    id: `${rowData.id}-settings`,
    ...
    if (rowData.cells) {
      ...
      Object.keys(rowData.cells).forEach((key) => {
        const itemInRowData = rowData.cells?.[key];
        if (itemInRowData) {
          itemInRowData.parentID = rowData.id;

          if (isHeader) itemInRowData.isInHeaderRow = true;
          if (isFooter) itemInRowData.isInFooterRow = true;
          itemInRowData.doesWrap = this.conf.styling?.cells?.wrapContent ?? false;

          // The individual cell generation process cycles through all the possible values a DataCell can implement that are provided by the rowData object, and sets them for each every single DataCell via the renderCell method.
          ...
          if (isHeader) {
            ...
            if (rowRenderOrderFromConf && rowRenderOrderFromConf.length > 0) {
              rowContent[rowRenderOrderFromConf.indexOf(itemInRowData.property)] = html`<th
  ...
  private renderCell(itemInRowData: IDataCellEntry): DataCell {
    ...
    const headerCellData = itemInRowData.isInHeaderRow ? itemInRowData as HeaderCell : null;
    const footerCellData = itemInRowData.isInFooterRow ? itemInRowData as FooterCell : null;

    // If the column is hidden, the cells are subsequentyly hidden.
    if (this.conf.renderOrder.columns.hiddenIDs?.includes(itemInRowData.property)) {
    ...
    const valueToString = itemInRowData.value !== undefined ? itemInRowData.value.toString() : null;
    ...
    if (itemInRowData.justifyContent !== undefined) outputCell.justifyContent = itemInRowData.justifyContent;
    if (itemInRowData.alignItems !== undefined) outputCell.alignItems = itemInRowData.alignItems;
    if (itemInRowData.isHidden !== undefined) outputCell.isHidden = itemInRowData.isHidden;
    if (itemInRowData.isInSubrow !== undefined) outputCell.isInSubrow = itemInRowData.isInSubrow;
    if (itemInRowData.doesWrap !== undefined) outputCell.wraps = itemInRowData.doesWrap;

    // Copy
    if (itemInRowData.isCopiable !== undefined) outputCell.isCopiable = itemInRowData.isCopiable;
    if (itemInRowData.isIDInCopyOutput !== undefined) outputCell.isIDInCopyOutput = itemInRowData.isIDInCopyOutput;
    // Copy in header && footer
    if (headerCellData?.onCopyInteractionFunction !== undefined) {
      outputCell.onCopyIconInteraction = () => { headerCellData.onCopyInteractionFunction?.(); };
    }
    if (footerCellData?.onCopyInteractionFunction !== undefined) {
      outputCell.onCopyIconInteraction = () => { footerCellData.onCopyInteractionFunction?.(); };
    }

    // Sorting
    if (itemInRowData.hasSortIcon !== undefined) outputCell.hasSortIcon = itemInRowData.hasSortIcon;
    if (outputCell.hasSortIcon) {
      ...
      if (headerCellData?.onSortInteractionFunction !== undefined) {
        outputCell.onSortIconInteraction = () => { headerCellData.onSortInteractionFunction?.(); };
      } else {
        ...
        outputCell.onSortIconInteraction = () => {
          const shouldInvert = this.hasAlreadySorted[outputCell.property] ?? false;
    ...
    if (itemInRowData.isDraggable !== undefined) outputCell.isDraggable = itemInRowData.isDraggable;
    if (headerCellData?.onRearrangeInteractionFunction !== undefined) { outputCell.onDragIconInteraction = () => { headerCellData.onRearrangeInteractionFunction?.(); }; }
    // TODO Missing behavior for dragging, due to lack of design for it.

    // Alert
    if (itemInRowData.isAlertCell !== undefined) outputCell.isAlertCell = itemInRowData.isAlertCell;
    if (itemInRowData.hasCustomAlertCellContent !== undefined) outputCell.hasCustomAlertCellContent = itemInRowData.hasCustomAlertCellContent;
    if (outputCell.isAlertCell && !outputCell.hasCustomAlertCellContent) {
      ...
      if (itemInRowData.onAlertClick !== undefined) outputCell.onContentIconInteraction = () => { itemInRowData.onAlertClick?.(); };
    }
    ...
    if (outputCell.isChevronCell) {
      outputCell.onContentIconInteraction = () => {
        ...
        if (itemInRowData.parentID !== undefined) {
          if (this.isSubrowOpen === undefined) this.isSubrowOpen = {};
          this.isSubrowOpen[itemInRowData.parentID] = !this.isSubrowOpen[itemInRowData.parentID] ?? true;
        }
      };
    }
    if (itemInRowData.parentID !== undefined) outputCell.isChevronOpen = this.isSubrowOpen[itemInRowData.parentID];

    // Settings
    if (itemInRowData.isSettingsCell !== undefined) outputCell.isSettingsCell = itemInRowData.isSettingsCell;
    if (itemInRowData.hasCustomSettingsCellContent !== undefined) outputCell.hasCustomSettingsCellContent = itemInRowData.hasCustomSettingsCellContent;
    if (itemInRowData.hasCustomThreeDotsCellContent !== undefined) outputCell.hasCustomThreeDotsCellContent = itemInRowData.hasCustomThreeDotsCellContent;
    if (outputCell.isSettingsCell && !outputCell.hasCustomSettingsCellContent && !outputCell.hasCustomThreeDotsCellContent) {
      if (itemInRowData.isInHeaderRow) {
        outputCell.onContentIconInteraction = () => { this.onTableSettingsClickFunction?.(); };
      } else {
        const onThreeDots = itemInRowData.onThreeDotsClick ?? this.onTableThreeDotsClickFunction;
        outputCell.onContentIconInteraction = () => { onThreeDots?.(); };
      }
    }
  ...
  protected async firstUpdated() {
    this.isRowDefaultOrderExternal = this.conf.renderOrder.rows.defaultOrder.length > 0;
    this.isColumnDefaultOrderExternal = this.conf.renderOrder.columns.defaultOrder.length > 0;
    ...
    if (this.conf.headerRow === undefined) this.conf = { renderOrder: emptyRenderOrder };
    ...
    this.renderedCells?.forEach((cell) => {
      if (this.renderedColumns) {
        ...
        if (cell.isInHeaderRow) this.renderedColumns[cell.property].columnID = cell.id;
      }
    });
  }
  ...
  protected updated(): void {
    this.isRowCustomOrderExternal = this.conf.renderOrder.rows.customOrder !== undefined && this.conf.renderOrder.rows.customOrder.length > 0;
    this.isColumnCustomOrderExternal = this.conf.renderOrder.columns.customOrder !== undefined && this.conf.renderOrder.columns.customOrder.length > 0;
  }


  render(): TemplateResult {
    let tableClass = '';
    tableClass += this.conf?.styling?.table?.areRowsSeparated ? 'wide-style ' : 'tight-style';
    return html`
